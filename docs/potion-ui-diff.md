...
### 2026-04-21 Configurable Practice Settings

이번 세션의 사용자 요청으로, 시작 전 우측 drawer에 `게임 설정` 탭을 추가한다.

- 설정 가능한 값은 `문항 수 40~100`, `응답 시간 3~10초`다.
- 각 줄은 `슬라이더 + 숫자 입력` 조합으로 구성한다.
- 숫자 입력은 범위를 벗어나면 가장 가까운 최소/최대값으로 보정한다.
- 실제 세션 생성, 남은 문항 표기, 문항 타이머는 이 설정값을 그대로 따른다.
- `문항 수`가 줄어들수록 우세 색상 확률을 높여서 `40문항 = 95%`, `100문항 = 80%`가 되도록 선형 보정한다.
- 결과 요약 카드는 같은 `문항 수 + 응답 시간` 설정 기록끼리만 비교하고, 전체 기록 리스트에서는 각 저장 기록의 설정값을 함께 표시한다.
